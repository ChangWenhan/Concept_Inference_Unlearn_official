...
def config():
    ...
    parser.add_argument("--out-dir", default='/home/cwh/Workspace/post-hoc-cbm-main/output/cifar100_output', type=str, help="Output folder")
    parser.add_argument("--pcbm-path",
                        default='/home/cwh/Workspace/post-hoc-cbm-main/output/cifar100_output/pcbm_cifar100__clip:RN50__multimodal_concept_clip:RN50_cifar100_recurse:1__lam:1e-05__alpha:0.99__seed:42_without.ckpt',
                        type=str, help="Trained PCBM module.")
    parser.add_argument("--concept-bank",
                        default="/home/cwh/Workspace/post-hoc-cbm-main/output/cifar100_output/multimodal_concept_clip:RN50_cifar100_recurse:1.pkl",
...
def calculate_accuracy(all_preds, all_labels):
    list_all_preds = []
    for i in all_preds:
        list_all_preds.append(np.argmax(i))
    correct = np.sum(list_all_preds == all_labels)
...
def train_hybrid(args, train_loader, val_loader, posthoc_layer, optimizer, num_classes):
    ...
    for epoch in range(1, args.num_epochs+1):
        ...
        acc = eval_model(args, posthoc_layer, val_loader, num_classes)
        latest_info["test_acc"] = acc
