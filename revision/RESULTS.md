...
- **对照类分析（残余信号归因）**：把同一 simple MIA 协议用在从未被遗忘的普通类上（同模型、同数据划分）：
  - CIFAR-10（deer 遗忘模型）：deer 0.016 **低于所有对照**（c0 0.058 / c5 0.036 / c7 0.022）
  - CIFAR-100（boy 遗忘模型）：boy 0.150（遗忘前 0.245）**落在对照区间内**（c12 0.090 / c13 0.200 / c42 0.085 / c73 0.115）
  - 结论：被遗忘类的成员信号不超过"普通类的固有水平"（100 类结构、每类仅 500 训练样本带来的天然可区分度），残余主要来自微调暴露而非未遗忘的类知识；仅 C100 绝对 gap 高于重训参考（0.045）这一点在正文如实说明
