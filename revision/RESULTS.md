...
### 对照类 MIA profile（残余信号归因实验）

**目的**：判断"遗忘后仍能区分成员/非成员"的信号，是目标类知识残留，还是模型集合对任意类的固有区分度。
**协议**：对数据集里**每一个类** c，成员 = c 的训练图、非成员 = c 的测试图，用完全相同的两种攻击各算一次，在三个模型上重复：原模型 / 遗忘后 / 重训。脚本 `revision/class_mia_profile.py`；完整数据保存于 `revision/work/class_mia_profile.{json,csv}` 和 `class_mia_profile_{cifar10,cifar100}.md`（逐类全部数值），每个 run 内另有 `class_mia.json`。

**CIFAR-10（全部 10 类）simple_gap**

| 模型 | 目标类 deer | 目标类排名 | 对照均值 | 对照范围 |
|---|---|---|---|---|
| 原模型 | 0.044 | 5/10 | 0.045 | 0.024–0.086 |
| 遗忘后 | **0.016** | **1/10（全类最低）** | 0.041 | 0.018–0.098 |
| 重训 | 0.016 | 4/10 | 0.021 | 0.005–0.038 |

**CIFAR-100（全部 100 类）simple_gap**

| 模型 | 目标类 boy | 目标类排名 | 对照均值 | 对照范围 |
|---|---|---|---|---|
| 原模型 | 0.245 | 98/100 | 0.119 | 0.025–0.290 |
| 遗忘后 | **0.150** | **71/100** | 0.119 | 0.025–0.285 |
| 重训 | 0.045 | 50/100 | 0.048 | 0.000–0.175 |

Fr 口径：遗忘后目标类 Fr = 1.000（C10）/ 0.998（C100），均为全类最高（对照均值 0.007 / 0.008）→ 目标类知识被完全移除。

**解读**：C10 上被遗忘类是全集**最难**区分成员的类；C100 上遗忘把 boy 从第 98 百分位降到第 71 百分位，回到普通类分布内部。残余信号与"从未被遗忘的普通类"同水平，属该模型集合的固有底噪（100 类头、每类仅 500 张训练图）与微调暴露，而非目标类知识残留。唯一需如实说明的是 C100 绝对 gap（0.150）高于重训参考（0.045）。
